...
export const workProjects: Project[] = [
  ...
  },
  {
    ...
    additionalDetails: `The Chardan website was built using a CMS called [Prismic](https://prismic.io) and a custom framework that used [handlebars.js](https://handlebarsjs.com/) to "render" dynamic content from the CMS and [jquery](https://jquery.com/) for a myriad different functions within the app. The custom framework was essentially a [Nodejs](https://nodejs.org) and [Express](https://expressjs.com) app that connected to the CMS, generated all of the pages from it and served those pages along with a few REST endpoints that the app used. It also would generate "microsites" which were essentially smaller websites with no more than two or three static pages with very minimal interactivity and upload them to a S3 bucket that was then served over a caching service. When I joined this project, it had already been running for quite a few years. I was responsible for creating new components that were requested by the design team. These components would have their data in the CMS and the templates and processing for that data would be done by the framework that had been built using tools like gulp, babel, handlebars and less. 
    
    Every time there was a new set of components to be built or new requirements came up, the Project manager and I would meet and discuss the requirements and how best to make those components such that whoever does content entry on that project finds it intuitive to add the content and ensure that components do not conflict with each other or affect the general layout of the website.
  ...
  },
  {
    ...
    additionalDetails: `The Humans web app is built with React, [D3](https://d3js.org), [GSAP](https://greensock.com/), [easy-peasy](https://easy-peasy.vercel.app/) and Typescript. The admin and back-end is powered by Strapi. GSAP powers most animations on the site with D3 powering the bubble simulation on its home page. 
